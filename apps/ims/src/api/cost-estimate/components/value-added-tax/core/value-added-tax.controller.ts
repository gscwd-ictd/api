...
export class ValueAddedTaxController implements ICrudRoutes {
  ...
  async create(@Body() data: CreateValueAddedTaxDto): Promise<ValueAddedTax> {
    return await this.valueAddedTaxService.create(data, () => new BadRequestException());
  }
  ...
  async findAll(
    ...
    return await this.valueAddedTaxService.findAll({ pagination: { page, limit } });
  }

  @Get(':id')
  async findById(@Param('id') id: string): Promise<ValueAddedTax> {
    return await this.valueAddedTaxService.findOneBy({ id }, () => new NotFoundException());
  }

  @Put(':id')
  async update(@Param('id') id: string, @Body() data: UpdateValueAddedTaxDto): Promise<UpdateResult> {
    return await this.valueAddedTaxService.update({ id }, data, () => new BadRequestException());
  }

  @Delete(':id')
  async delete(@Param('id') id: string): Promise<DeleteResult> {
    return this.valueAddedTaxService.delete({ id }, () => new BadRequestException());
  }
}
