...
export class StatsService {
  ...
  async countAllPendingApplicationsForManager(employeeId: string) {
    ...
    const employeesUnderOrgId = await this.employeeService.getEmployeesByOrgId(employeeId);
    ...
    const pendingPassSlipsCount = (
      ...
      SELECT count(ps.pass_slip_id) passSlipCount
        FROM pass_slip ps INNER JOIN pass_slip_approval psa ON psa.pass_slip_id_fk = ps.pass_slip 
    ...
    const pendingOvertimesCount = (
      await this.passSlipService.rawQuery(
        `
    SELECT count(oa.overtime_application_id) overtimeApplicationCount
    ...
    const pendingLeavesCount = (
      await this.passSlipService.rawQuery(
        `
        SELECT count(leave_application_id) leaveCount FROM leave_applications WHERE supervisor_id_fk = ? AND status = ?;
    `,
