...
export class EquipmentCostController implements ICrudRoutes {
  ...
  async create(@Body() data: CreateEquipmentCostDto): Promise<EquipmentCost> {
    return await this.equipmentCostService.getProvider().create(data, () => new BadRequestException());
  }
  ...
  ): Promise<Pagination<EquipmentCost>> {
    return await this.equipmentCostService.getProvider().findAll({ pagination: { page, limit } });
  }

  @Get(':id')
  async findById(@Param('id') id: string): Promise<EquipmentCost> {
    return await this.equipmentCostService.getProvider().findOneBy({ id }, () => new NotFoundException());
  }

  @Put(':id')
  async update(@Param('id') id: string, @Body() data: UpdateEquipmentCostDto): Promise<UpdateResult> {
    return await this.equipmentCostService.getProvider().update({ id }, data, () => new BadRequestException());
  }

  @Delete(':id')
  async delete(@Param('id') id: string): Promise<DeleteResult> {
    return await this.equipmentCostService.getProvider().delete({ id }, () => new BadRequestException());
  }
}
