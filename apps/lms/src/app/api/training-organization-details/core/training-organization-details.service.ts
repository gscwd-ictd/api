...
@Injectable()
export class TrainingOrganizationDetailsService extends CrudHelper<TrainingOrganizationDetails> {
  constructor(private readonly crudService: CrudService<TrainingOrganizationDetails>, private readonly datasource: DataSource) {
    super(crudService);
  }

  //HR create training individual details and distribute slots to selected managers
  async addTraining(data: CreateTrainingOrganizationDetailsDto) {
    try {
      //transaction results
      const results = await this.datasource.transaction(async (entityManager) => {
        //deconstruct dto //nomineeQualifications, trainingDistribution,
        const { courseContent, ...rest } = data;

        //insert training details
        const training = await this.crudService.transact<TrainingOrganizationDetails>(entityManager).create({
          dto: {
            ...rest,
